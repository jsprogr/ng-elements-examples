...
import {AvDataService} from '../../services/av-data.service';
...
export class ChatComponent implements AfterViewInit {
  ...
  constructor(private avDataService: AvDataService) { }
  ...
  async call(): Promise<void> {
    ...
    try {
      ...
      this.avDataService.sendMessage({type: 'offer', data: offer});
    } catch (err) {
  ...
  hangUp(): void {
    this.avDataService.sendMessage({type: 'hangup', data: ''});
    this.closeVideoCall();
  }
  ...
  private addIncominMessageHandler(): void {
    this.avDataService.connect();

    // this.transactions$.subscribe();
    this.avDataService.messages$.subscribe(
      msg => {
  ...
  private handleOfferMessage(msg: RTCSessionDescriptionInit): void {
    ...
    this.peerConnection.setRemoteDescription(new RTCSessionDescription(msg))
      ...
      this.avDataService.sendMessage({type: 'answer', data: this.peerConnection.localDescription});
  ...
  private handleICECandidateEvent = (event: RTCPeerConnectionIceEvent) => {
    console.log(event);
    if (event.candidate) {
      this.avDataService.sendMessage({
        type: 'ice-candidate',
